    ...
    .then((answers) => {
      if (answers.action === "View All Employees") {
        ...
        db.query("SELECT title FROM roles", function (error, results) {
          ...
          db.query(
            "SELECT first_name, last_name FROM employee WHERE manager = TRUE",
                ...
                inquirer
                  ...
                  .then((answers) => {
                    ...
                    if (answers.manager === "none") {
                      ...
                      db.query(
                        `SELECT id FROM roles WHERE title = '${answers.empRole}'`,
                        function (error, results) {
                          if (error) {
                            console.log(error);
                          }
                          ...
                          db.query(
                            `INSERT INTO employee (first_name, last_name, role_id, manager_id, manager) VALUES ('${answers.firstName}', '${answers.lastName}', ${selectedRole}, ${selectedManager}, ${isManager})`,
                            (err, results) => {
                              if (err) {
                                console.log(err);
                              }
                              console.log(
                                `New employee added to the employee table`
                              );
                              console.log(" ");
                              startPrompt();
                            }
            ...
            db.query("SELECT title FROM roles", function (error, results) {
              ...
              inquirer
                ...
                .then((answers) => {
                  ...
                  db.query(
                    `SELECT id FROM roles WHERE title = '${answers.updatedRole}'`,
                    function (error, results) {
                      if (error) {
                        console.log(error);
                      }
                      ...
                      db.query(
                        `UPDATE employee SET role_id = ${roleId} WHERE first_name = '${nameArray[0]}' AND last_name = '${nameArray[1]}'`,
                        function (error, results) {
                          if (error) {
                            console.log(error);
                          }
        ...
        db.query(
          ...
          function (error, results) {
            ...
            inquirer
              ...
              .then((answers) => {
                db.query(
                  `SELECT id FROM department WHERE department_name = '${answers.roleDept}'`,
                  (err, results) => {
                    let roleId = results[0].id;

                    db.query(
                      `INSERT INTO roles (title, salary, department_id) VALUES ('${answers.newRole}', '${answers.salary}', ${roleId})`,
                      (err, results) => {
                        if (err) {
                          console.log(err);
                        }
                        console.log(
                          `${answers.newRole} role added to the roles table`
                        );
                        console.log(" ");
                        startPrompt();
                      }
        ...
        inquirer
          ...
          .then((answers) => {
            ...
            db.query(
              `INSERT INTO department (department_name) VALUES ('${dept}')`,
              function (err, results) {
                if (err) {
                  console.log(err);
                }
